class Stage < ActiveRecord::Base
  ...
  def effective_configuration(key=nil)
    ...
    cleaned_project_configs = project_configs.to_a.delete_if{|x| my_configs.to_a.collect(&:name).collect(&:to_s).include?(x.name.to_s) }
  ...
  def lock
    Stage.update(self.id, locked: 1)
    self.reload
  end

  def unlock
    Stage.update(self.id, locked: 0)
    Stage.update(self.id, locked_by_deployment_id: nil)
    self.reload
  end
