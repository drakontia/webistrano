...
class DeploymentTest < ActiveSupport::TestCase
  ...
  test "completed and status on error" do
  ...
  test "completed and status on success" do
    assert !@deployment.completed?
    assert !@deployment.success?
    assert_equal 'running', @deployment.status
  ...
  test "validation depends on stage ready to deploy" do
  ...
  test "check of stage prompt configuration in validation" do
  ...
  test "completion alertshper mail when no alert emails set" do
  ...
  test "completion alerts per mail when alert emails set on error" do
    # prepare ActionMailer
    emails = prepare_email
  ...
  test "excluded hosts_accessor" do
  ...
  test "cannot exclude all hosts" do
    stage = FactoryGirl.create(:stage)
    host = FactoryGirl.create(:host)
  ...
  test "cancel handles pid gone" do
    deployment = FactoryGirl.create(:deployment, :pid => 5542, :stage => FactoryGirl.create(:role, :name => 'app').stage, :completed_at => nil)
    assert deployment.cancelling_possible?, deployment.inspect
  ...
  test "validation fails if stage locked" do
  ...
  test "validation does not fails if stage locked but we override" do
    stage = FactoryGirl.create(:role, :name => 'app').stage
    stage.lock
  ...
  test "completing with error clears the stage lock" do
    stage = FactoryGirl.create(:role, :name => 'app').stage
  ...
  test "completing success clears the stage lock" do
    stage = FactoryGirl.create(:role, :name => 'app').stage
    deployment = FactoryGirl.create(:deployment, :stage => stage, :completed_at => nil)
    assert deployment.running?
  ...
  test "completing cancelled clears thelstage lock" do
    stage = FactoryGirl.create(:role, :name => 'app').stage
  ...
  test "effective and prompt config" do
    stage = FactoryGirl.create(:role, :name => 'app').stage
